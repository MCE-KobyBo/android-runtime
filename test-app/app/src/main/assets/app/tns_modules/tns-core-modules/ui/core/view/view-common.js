...
var ViewCommon = (function (_super) {
    ...
    ViewCommon.resolveSizeAndState = function (size, specSize, specMode, childMeasuredState) {
        ...
        switch (specMode) {
            ...
            case view_base_1.layout.AT_MOST:
                if (specSize < size) {
                    result = Math.round(specSize + 0.499) | view_base_1.layout.MEASURED_STATE_TOO_SMALL;
                }
                break;
        ...
        return Math.round(result + 0.499) | (childMeasuredState & view_base_1.layout.MEASURED_STATE_MASK);
    };
    ...
    ViewCommon.measureChild = function (parent, child, widthMeasureSpec, heightMeasureSpec) {
        ...
        var measureHeight = 0;
        if (child && !child.isCollapsed) {
            var width = view_base_1.layout.getMeasureSpecSize(widthMeasureSpec);
            var widthMode = view_base_1.layout.getMeasureSpecMode(widthMeasureSpec);
            var height = view_base_1.layout.getMeasureSpecSize(heightMeasureSpec);
            var heightMode = view_base_1.layout.getMeasureSpecMode(heightMeasureSpec);
            ...
            var childWidthMeasureSpec = ViewCommon.getMeasureSpec(width, widthMode, horizontalMargins, child.effectiveWidth, style.horizontalAlignment === "stretch");
            var childHeightMeasureSpec = ViewCommon.getMeasureSpec(height, heightMode, verticalMargins, child.effectiveHeight, style.verticalAlignment === "stretch");
            if (view_base_1.traceEnabled()) {
